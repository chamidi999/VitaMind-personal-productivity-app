        ...
        <div key={column.id} className="bg-card/50 border border-white/5 rounded-3xl flex flex-col max-h-[80vh]">
          <div className="p-5 border-b border-white/5 flex justify-between items-center bg-card/30 rounded-t-3xl">
            <div className="flex items-center gap-3">
              ...
              <h3 className="font-bold text-sm uppercase tracking-wider text-gray-400">{column.title}</h3>
              <span className="bg-white/5 px-2 py-0.5 rounded-full text-xs text-gray-500">
          ...
          <div className="flex-1 overflow-y-auto p-4 space-y-4 custom-scrollbar">
              ...
              >
                <div className="flex justify-between items-start mb-3">
                  ...
                  <div className="flex items-center gap-2 relative z-20">
                    ...
                    <button className="text-gray-600 hover:text-gray-400 opacity-0 group-hover:opacity-100 transition-opacity p-1.5">
                      <MoreVertical size={16} />
                    </button>
                  </div>
                </div>
                <h4 className="font-semibold text-sm mb-2 text-white/90 leading-tight">{task.title}</h4>
                ...
                <div className="mt-4 flex gap-2">
                   ...
                   {column.id !== 'in-progress' && (
                     <button 
                      ...
                      className="text-[10px] font-bold text-royal hover:text-blue-400 transition-colors"
                     >
                       Move to In Progress
                     </button>
                   )}
            ...
            {tasks.filter(t => t.status === column.id).length === 0 && (
              <div className="border border-dashed border-white/5 rounded-2xl p-8 flex flex-col items-center justify-center text-center opacity-30">
                ...
                <p className="text-xs">No tasks in {column.title}</p>
              </div>
            )}
          </div>
        </div>
